junction.support = function (reads, junctions = NULL, bwa = NULL, ref = NULL, pad = 500, 
    ...
    ...) 
{
    ...
    if (!realign) {
        ...
        out = dt2gr(setDT(dplyr::inner_join(as.data.frame(reads),
                                            as.data.frame(unique(jn$dt[, .(qname, junction.id = subject.id)])), by = "qname")), seqlengths = sl)
        return(out)
    }
...
overwritefun("junction.support", "junction.support", "skitools")

junction.support = skitools::junction.support
