...
suppressWarnings(expr = {
    suppressMessages(expr = {
        suppressPackageStartupMessages(expr = {
            withr::with_libpaths(priv_lib, library(igraph))
            library(gGnome)
            withr::with_libpaths(priv_lib, library(gTrack))
            library(gUtils)
            library(colorspace)
            library(MASS)
            library(cowplot)
            ...
            library(plyr)
            library(sinaplot)            
            withr::with_libpaths(priv_lib, library(dplyr))
            ## library(dplyr)
            library(tidyverse)
            withr::with_libpaths(lab_lib, library(VariantAnnotation))
            ## withr::with_libpaths("/gpfs/commons/groups/imielinski_lab/lib/R-3.4.1", library(gGnome))
            withr::with_libpaths(priv_lib, library(Flow))
            library(skitools)
            library(skidb)
            library(rtracklayer)
            library(gTrack)
